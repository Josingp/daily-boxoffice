...
import manualData from '../manual_data.json';
...
export const fetchMoviePoster = async (movieName: string): Promise<string> => {
  // [NEW] 수동 설정 우선 확인 (공백 무시)
  const cleanName = movieName.replace(/\s+/g, '');
  const key = Object.keys(MANUAL_DATA).find(k => k.replace(/\s+/g, '') === cleanName);
